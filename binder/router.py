	...
	def urls(self):
		...
		for route, view in self.route_views.items():
			...
			if route.list_endpoint:
				urls.append(django.urls.re_path(r'^{}/$'.format(route.route), view.as_view(), {'router': self}, name=name))
			if route.detail_endpoint:
				urls.append(django.urls.re_path(r'^{}/(?P<pk>[0-9]+)/$'.format(route.route), view.as_view(), {'router': self}, name=name))

			# History views
			if view.model and hasattr(view.model, 'Binder') and view.model.Binder.history:
				urls.append(django.urls.re_path(r'^{}/(?P<pk>[0-9]+)/history/$'.format(route.route), view.as_view(), {'history': 'normal', 'router': self}, name=name))
				urls.append(django.urls.re_path(r'^{}/(?P<pk>[0-9]+)/history/debug/$'.format(route.route), view.as_view(), {'history': 'debug', 'router': self}, name=name))

			# File field endpoints
			for ff in view.file_fields:
				urls.append(django.urls.re_path(r'^{}/(?P<pk>[0-9]+)/{}/$'.format(route.route, ff),
			...
			for m in dir(view):
				...
				if hasattr(method, 'detail_route') or hasattr(method, 'list_route'):
					...
					if hasattr(method, 'detail_route'):
						urls.append(django.urls.re_path(r'^{}/(?P<pk>[0-9]+)/{}/{}$'.format(route.route, route_name, extra),
								view.as_view(), kwargs, name='{}.{}'.format(name, route_name)))
					if hasattr(method, 'list_route'):
						urls.append(django.urls.re_path(r'^{}/{}/{}$'.format(route.route, route_name, extra),
								view.as_view(), kwargs, name='{}.{}'.format(name, route_name)))
