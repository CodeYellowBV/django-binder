...
from .exceptions import BinderException, BinderFieldTypeError, BinderFileSizeExceeded, BinderForbidden, \
    BinderImageError, BinderImageSizeExceeded, BinderInvalidField, BinderIsDeleted, BinderIsNotDeleted, \
    BinderMethodNotAllowed, BinderNotAuthenticated, BinderNotFound, BinderReadOnlyFieldError, BinderRequestError, \
    BinderValidationError, BinderFileTypeIncorrect, BinderInvalidURI
from . import history
...
def parse_pk(pk):
    """
    int() if pk is integer, or pk else. This is necessary to get multiput to work
    :param pk:
    :return:
    """
    if re.match(r"^[-+]?[0-9]+$", str(pk)):
        return int(pk)
    return pk
...
class ModelView(View):
        ...
        if not f:
            ...
            for field_filter_cls in FieldFilter.__subclasses__():
                for field_cls in field_filter_cls.fields:
                    if f.get(field_cls):
                        raise ValueError(
                            'Field-Filter mapping conflict: {} vs {}'.format(field_filter_cls.name, field_cls.name))
                    else:
    ...
    def _annotate_objs(self, datas_by_id, objs_by_id):
        ...
        for field_name in self.m2m_fields:
            ...
            for other, this in remote_field.model.objects.filter(**{remote_field.name + '__pk__in': pks}).values_list(
                    'pk', remote_field.name + '__pk'):
    ...
    def _follow_related(self, fieldspec):
        ...
        if not field.is_relation:
            raise BinderRequestError(
                'Field is not a related object {{{}}}.{{{}}}.'.format(self.model.__name__, fieldname))
    ...
    def _filter_field(self, queryset, field_name, qualifier, value, invert, partial=''):
        if field_name in self.annotations:
            ...
            try:
                ...
                raise BinderRequestError(
                    'Unknown field in filter: {{{}}}.{{{}}}.'.format(self.model.__name__, field_name))
    ...
    def get(self, request, pk=None, withs=None):
        ...
        if pk:
            queryset = queryset.filter(pk=parse_pk(pk))
        ...
        if django.conf.settings.DEBUG and 'debug' in request.GET:
            ...
            for connection in django.db.connections:
                debug['queries'][connection] = ['{}s: {}'.format(q['time'], q['sql'].replace('"', '')) for q in
                                                django.db.connections[connection].queries]

                qc += len(django.db.connections[connection].queries)
            debug['query_count'] = qc

        response_data = {'data': data, 'with': extras, 'with_mapping': extras_mapping,
                         'with_related_name_mapping': extras_reverse_mapping, 'meta': meta, 'debug': debug}
    ...
    def _store_field(self, obj, field, value, request, pk=None):
                            ...
                            raise BinderValidationError({
                                model_name: {
                                    obj.pk if pk is None else pk: {
                                        f.name: [{
                                            'code': 'max_length',
                                            'message': 'Ensure this value has at most {} characters (it has {}).'.format(
                                                f.max_length, len(value)),
        ...
        for f in list(self.model._meta.many_to_many) + list(self._get_reverse_relations()):
            if f.name == field:
                ...
                ids -= set(obj._meta.get_field(field).remote_field.model.objects.filter(id__in=ids).values_list('id',
                                                                                                                flat=True))
                if ids:
        ...
        for (model, mid), values in objects.items():
            for field in filter(lambda f: f.one_to_many, model._meta.get_fields()):
                if field.name in values:
                    if not isinstance(values[field.name], list) or not all(
                            isinstance(v, int) for v in values[field.name]):
                    ...
                    if field.name in values:
                        if values[field.name] is not None:
                            if not isinstance(values[field.name], int):
                                raise BinderRequestError(
                                    'with.{}.{} should be an integer'.format(model.__name__, field.name))
    ...
    def _multi_put_save_objects(self, ordered_objects, objects, request):
        ...
        for model, oid in ordered_objects:
            ...
            for field in model._meta.many_to_many:
                if field.name in values:
                    values[field.name] = [
                        (new_id_map[(field.related_model, multiput_get_id(i))] if multiput_get_id(i) < 0 else i) for i
                        in values[field.name]]
    ...
    def put(self, request, pk=None):
        ...
        try:
            obj = self.get_queryset(request).select_for_update().get(pk=parse_pk(pk))
            # Permission checks are done at this point, so we can avoid get_queryset()
            old = self._get_objs(
                annotate(self.model.objects.filter(pk=parse_pk(pk))),
                request,
            )[0]
        except ObjectDoesNotExist:
            raise BinderNotFound()
        ...
        meta['with'], meta['with_mapping'], meta['with_related_name_mapping'] = self._get_withs([new['id']],
                                                                                                request=request,
                                                                                                withs=None)
    ...
    def delete(self, request, pk=None, undelete=False):
        ...
        try:
            if request.body not in (b'', b'{}'):
                raise BinderRequestError(
                    '{}DELETE body must be empty or empty json object.'.format('UN' if undelete else ''))
        except ValueError:
            pass
    ...
    def dispatch_file_field(self, request, pk=None, file_field=None):
        ...
        if isinstance(pk, self.model):
            ...
            try:
                obj = self.get_queryset(request).get(pk=parse_pk(pk))
            except ObjectDoesNotExist:
                raise BinderNotFound()
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                raise BinderRequestError(
                    'File POST should use multipart/form-data (with an arbitrary key for the file data).')
            ...
            try:
                ...
                if isinstance(field, models.fields.files.ImageField):
                    ...
                    if not format in ('png', 'gif', 'jpeg'):
                        raise BinderFileTypeIncorrect(
                            [{'extension': t, 'mimetype': 'image/' + t} for t in ['jpeg', 'png', 'gif']])
                    ...
                    if width > max_width or height > max_height:
                        ...
                        logger.info(
                            'image dimensions ({}x{}) exceeded ({}, {}), resizing.'.format(width, height, max_width,
                                                                                           max_height))
                        file = io.BytesIO()
                ...
                logger.info('POST updated {}[{}].{}: {} -> {}'.format(self._model_name(), pk, file_field_name, old_hash,
                                                                      new_hash))
    ...
    def view_history(self, request, pk=None, **kwargs):
        ...
        changesets = history.Changeset.objects.filter(id__in=set(
            history.Change.objects.filter(model=self.model.__name__, oid=pk).values_list('changeset_id', flat=True)))
        if debug:
